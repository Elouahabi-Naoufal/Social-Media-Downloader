...
def download():
    ...
    try:
        ...
        # Use direct download for videos, server download for audio
        ...
        if result['success']:
            # Save to history only for server downloads
            ...
            return jsonify({
                ...
                'media_type': result.get('media_type', download_type),
                'download_url': url_for('download_file', filename=result['filename']) if download_type == 'audio' else None,
